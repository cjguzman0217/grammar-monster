/*
This is a derivative work of https://github.com/GitbookIO/hunspell-spellchecker
Changes, if any, are stated in this file.
This work is licensed under Apache License, Version 2.0, January 2004, http://www.apache.org/licenses/
 */
...
// =====================================================================================
// ADDITION: LOAD AND PARSE DICT IN BROWSER
// This is a "change" as required in the Apache-2.0 license
// =====================================================================================



/**
 * Loads a text-based resource from a given URL
 * @param url {string}
 * @param callback {function}
 */
const loadAsText = (url, callback) => {
  ...
  fetch(request)
    ...
    .then(file => {
      ...
      reader.addEventListener("load", () => callback(undefined, reader.result), false);
      reader.readAsText(file)
    })
}

/**
 * Loads a dict from given urls and creates a new SpellChecker instance with this given
 * dict as source.
 * @param urls.aff {string}
 * @param urls.dic {string}
 * @param callback
 */
Spellchecker.createWithDict = (urls, callback) => {
  loadAsText(urls.aff, (err, aff) => {
    if (err) { return callback(err) }

    loadAsText(urls.dic, (err, dic) => {
      if (err) { return callback(err) }

      try {
        const spellchecker = new Spellchecker()
        const parsedDict = spellchecker.parse({ aff, dic })

        spellchecker.use(parsedDict)
        return callback(undefined, spellchecker)
      } catch(loadErr) {
        callback(loadErr)
      }
    })
